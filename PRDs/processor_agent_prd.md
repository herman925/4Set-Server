...
4. **Upload Engine**
   ...
   - **Data Overwrite Protection** (Configurable): Before updating existing submissions, optionally validate that no non-exception fields would be overwritten with different values:
     - **Configuration**: Controlled via `config/agent.json` → `validation.enableDataOverwriteProtection` (default: `true` for backward compatibility)
       - When `true`: Enforces data overwrite protection (original behavior)
       - When `false`: Allows full data overwrites without conflict checks
     - **Exception Fields** (allowed to overwrite when protection is enabled): `student-id`, `child-name`, `school-id`, `district`, `class-id`, `class-name`, `computerno` (administrative/metadata fields)
     - **Protected Fields** (when protection is enabled): All other assessment data fields (ERV, CM, CWR, etc.)
     - **Conflict Detection** (when protection is enabled): Compares incoming JSON against existing JotForm submission
     ...
     - **Conflict Handling** (when protection is enabled): Files with data overwrite conflicts are logged with `DATA_OVERWRITE_DIFF` level and moved to `Unsorted/` for manual review
     - **When protection is disabled**: All data updates proceed without conflict validation, allowing full overwrites. Useful for data correction workflows where human operators take responsibility for data integrity.
   ...
   - On failure (including overwrite conflicts when protection is enabled), relocate files to `PDF Form Data\Unsorted\` with diagnostic JSON and queue remediation telemetry.
